...
async function getRandomNumberRequest() {
    try {
        const request = await fetch('https://async-demo.herokuapp.com/stable?maxRandom=20&prob=50');
        if (!request.ok) {
...
async function randomNumber(num) {
    try {
        const response = await fetch(`https://async-demo.herokuapp.com/stable?maxRandom=${num}&prob=5`);
        if (!response.ok) {
...
async function getLastUser() {
    try {
        ...
        randomNum = +response === 0 ? response : response - 1;
